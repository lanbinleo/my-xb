...
**降级处理**:
- 当某科目的成绩详情接口失败（如"该课程成绩未同步到移动端"）时，不会终止整个计算，而是降级为仅使用官方总分计算该科目（GPA 贡献与完整抓取一致，仅缺分项明细），并输出警告 `Used official score only for ...`
- 若官方总分也不存在，该科目分数记为 NaN 并排除出 GPA 计算，输出警告 `Skipped ...`
- 批量接口（学期列表、科目列表、学期总成绩）失败仍视为致命错误
